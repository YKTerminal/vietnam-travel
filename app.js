...
const airportGeo = {
  CAN: { lat: 23.3924, lng: 113.2988, name: "广州白云机场" },
  SGN: { lat: 10.8188, lng: 106.6520, name: "胡志明新山一机场" },
  PQC: { lat: 10.1698, lng: 103.9931, name: "富国岛机场" },
  HAN: { lat: 21.2212, lng: 105.8070, name: "河内内排机场" },
  SZX: { lat: 22.6393, lng: 113.8107, name: "深圳宝安机场" },
...
function renderFlightMaps() {
  if (!window.L) return;
  $$(".flight-card:not(.flight-card--placeholder)").forEach((card) => {
    const flights = journeyFlights(card.dataset.journey);
    if (!flights.length) return;
    const first = flights[0];
    const last = flights[flights.length - 1];
    const dep = airportGeo[first.departure?.airportCode];
    const arr = airportGeo[last.arrival?.airportCode];
    if (!dep || !arr) return;
    const host = document.createElement("div");
    host.className = "flight-minimap";
    ...
    map.fitBounds([[dep.lat, dep.lng], [arr.lat, arr.lng]], { padding: [22, 22] });
